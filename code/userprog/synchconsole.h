...
class SynchConsoleInput : public CallBackObj {
  public:
    ...
    int ReadStr(char* buffer,int length);
  private:
    ConsoleInput_ *consoleInput;	// the hardware keyboard
...
class SynchConsoleOutput : public CallBackObj {
  public:
    ...
    void PrintStr(char* buffer,int length = 128);
  private:
    ConsoleOutput_ *consoleOutput;// the hardware display
